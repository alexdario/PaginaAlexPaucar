...
if(!empty($_POST)){
	if(isset($_POST["username"]) &&isset($_POST["fullname"]) &&isset($_POST["email"]) &&isset($_POST["password"]) &&isset($_POST["confirm_password"])){
		if($_POST["username"]!=""&& $_POST["fullname"]!=""&&$_POST["email"]!=""&&$_POST["password"]!=""&&$_POST["password"]==$_POST["confirm_password"]){
			...
			if($found){
				print "<script>alert(\"El usuario ya consta en lista por favor registrese con otro nombre.\");window.location='../registro.php';</script>";
			}
			...
			$query = $con->query($sql);
			if($query!=null){
				print "<script>alert(\"Registro hecho con exito\");window.location='../login.php';</script>";
			}
		}
	}
}
